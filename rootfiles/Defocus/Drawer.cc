...
//	TString title = "xBcOut-cluster_x:cluster_z";
	...
	double cx=-30,cy=5;
	TString cut = Form("abs(cluster_hitpos_x-%f)<25&&abs(cluster_hitpos_y-%f)<15&&ntBcOut==1&&nhTpc>8",cx,cy);
//	cut = Form("ntBcOut==1&&nhTpc>8");
	TCanvas* c1 = new TCanvas("c1","c1",1600,800);
	c1->Divide(2,1);
	c1->cd(1);
	tree->Draw(Form("xCorVec:cluster_hitpos_z>>%s",title2.Data()),cut,"col");
	h2->GetXaxis()->SetTitle("z [mm]");
	h2->GetYaxis()->SetTitle("x residual [mm]");
	c1->cd(2);
	tree->Draw(Form("xCorRes:cluster_hitpos_z>>%s",title.Data()),cut,"col");
