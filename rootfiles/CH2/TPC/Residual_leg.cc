...
//		TFile* file = new TFile("run05641_DstTPCHSKuramaHelixTracking.root");
...
//	TFile* file2 = new TFile(Form("run0%d_hists_nocut.root",runnum),"recreate");
	TFile* file2 = new TFile(Form("run0%d_histsScaled.root",runnum),"recreate");
...
//	TCut VertCut = "abs(x0Tpc)<25&&abs(y0Tpc)<25&&(abs(v0Tpc)>0.1||abs(u0Tpc)>0.1)";
	TCut VertCut = "helix_flag<400&&mom0<0.6&&mom0>0.35&&abs(helix_dz)>-0.01&&path>200&&(abs(hitpos_x)>30||abs(hitpos_y)>30||abs(hitpos_z+143)>50)&&abs(sqrt(helix_cx*helix_cx+helix_cy*helix_cy)-helix_r)<20";
...
void Residual(){
	cout<<"MakePadResidualHists()"<<endl;
	cout<<"MakeResidualHists()"<<endl;
}
...
*/	
	...
	c3->cd(1);
	RYDz->Draw("AP");
	RYDz->SetMarkerStyle(25);
	c3->cd(2);
	RYDzCl->Draw("AP");
	RYDzCl->SetMarkerStyle(25);
}
